...
interface Fine { date: string; amount: number; description: string; }
interface Relocation { date: string; from: string; to: string; mileage: number; rate: number; }
...
export default function NewReportPage() {
  ...
  const [newFineDate, setNewFineDate] = useState("");
  const [newFineAmount, setNewFineAmount] = useState<number | "">("");
  const [newFineDesc, setNewFineDesc] = useState("");
  
  // Порожний пробег (relocations)
  const [relocations, setRelocations] = useState<{date: string; from: string; to: string; mileage: number; rate: number}[]>([]);
  const [newRelocDate, setNewRelocDate] = useState("");
  const [newRelocFrom, setNewRelocFrom] = useState("");
  const [newRelocTo, setNewRelocTo] = useState("");
  const [newRelocMileage, setNewRelocMileage] = useState<number | "">("");
  const [newRelocRate, setNewRelocRate] = useState<number | "">(7);
  ...
  const totalRelocations = relocations.reduce((sum, r) => sum + ((Number(r.mileage) || 0) * (Number(r.rate) || 0)), 0); // Порожний пробег (-)
  ...
  const totalToPay = totalDriverPay + totalExpenses + totalExtraWorks - totalPayments - totalFines - totalRelocations;
...
${comment ? `Комментарий: ${comment}` : ""}`;
  ...
  const addFine = () => {
    if (newFineAmount) {
      setFines([...fines, { date: newFineDate || new Date().toISOString().slice(0,10), amount: Number(newFineAmount), description: newFineDesc }]);
      setNewFineDate(""); setNewFineAmount(""); setNewFineDesc("");
    }
  };
  
  const addRelocation = () => {
    if (newRelocMileage) {
      setRelocations([...relocations, { date: newRelocDate || new Date().toISOString().slice(0,10), from: newRelocFrom, to: newRelocTo, mileage: Number(newRelocMileage), rate: Number(newRelocRate) || 7 }]);
      setNewRelocDate(""); setNewRelocFrom(""); setNewRelocTo(""); setNewRelocMileage(""); setNewRelocRate(7);
    }
  };
    ...
    <div className="p-4 md:p-6 max-w-6xl mx-auto">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-slate-800 rounded-xl p-4 border border-red-500/30">
          <h2 className="font-semibold text-red-400 mb-1">⚠️ Штрафы</h2>
          <p className="text-xs text-slate-400 mb-2">Удержания из зарплаты</p>
          {fines.map((f, i) => (
            <div key={i} className="flex justify-between bg-slate-700/50 rounded px-3 py-1 mb-1 text-sm">
              <span>{f.date && <span className="text-slate-500 mr-1">{f.date.slice(5,10).split('-').reverse().join('.')}</span>}{f.description || 'Штраф'}</span>
              <div className="flex gap-2">
                <span className="text-red-400">−{f.amount.toLocaleString()} ₽</span>
          ...
          <div className="space-y-2 sm:space-y-0 mt-2">
            <div className="flex gap-2 mb-2 sm:mb-0">
              <input type="date" value={newFineDate} onChange={e => setNewFineDate(e.target.value)} className="flex-1 sm:flex-none bg-slate-700 text-white rounded px-2 py-2 sm:py-1 border border-slate-600 text-sm" />
              <input placeholder="Описание..." value={newFineDesc} onChange={e => setNewFineDesc(e.target.value)} className="flex-1 min-w-0 bg-slate-700 text-white rounded px-3 py-2 sm:py-1 border border-slate-600 text-sm" />
            </div>
            <div className="flex gap-2">
              <input type="number" placeholder="Сумма ₽" value={newFineAmount} onChange={e => setNewFineAmount(e.target.value ? Number(e.target.value) : "")} className="flex-1 bg-slate-700 text-white rounded px-3 py-2 sm:py-1 border border-slate-600 text-sm" />
              <button onClick={addFine} disabled={!newFineAmount} className="bg-red-600 disabled:bg-slate-600 text-white px-4 sm:px-3 py-2 sm:py-1 rounded min-h-[44px] sm:min-h-0"><Plus className="w-4 h-4" /></button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-800 rounded-xl p-4 border border-amber-500/30">
          <h2 className="font-semibold text-amber-400 mb-1">🛣️ Порожний пробег</h2>
          <p className="text-xs text-slate-400 mb-2">Перегоны без груза (вычитается)</p>
          {relocations.map((r, i) => (
            <div key={i} className="flex justify-between bg-slate-700/50 rounded px-3 py-1 mb-1 text-sm">
              <span>{r.date && <span className="text-slate-500 mr-1">{r.date.slice(5,10).split('-').reverse().join('.')}</span>}{r.from}→{r.to} ({r.mileage} км)</span>
              <div className="flex gap-2">
                <span className="text-amber-400">−{(r.mileage * r.rate).toLocaleString()} ₽</span>
                <button onClick={() => setRelocations(relocations.filter((_, j) => j !== i))} className="text-slate-500"><Trash2 className="w-3 h-3" /></button>
              </div>
            </div>
          ))}
          <div className="space-y-2 mt-2">
            <div className="flex gap-2">
              <input type="date" value={newRelocDate} onChange={e => setNewRelocDate(e.target.value)} className="flex-none bg-slate-700 text-white rounded px-2 py-2 sm:py-1 border border-slate-600 text-sm w-32" />
              <input placeholder="Откуда" value={newRelocFrom} onChange={e => setNewRelocFrom(e.target.value)} className="flex-1 min-w-0 bg-slate-700 text-white rounded px-2 py-2 sm:py-1 border border-slate-600 text-sm" />
              <input placeholder="Куда" value={newRelocTo} onChange={e => setNewRelocTo(e.target.value)} className="flex-1 min-w-0 bg-slate-700 text-white rounded px-2 py-2 sm:py-1 border border-slate-600 text-sm" />
            </div>
            <div className="flex gap-2">
              <input type="number" placeholder="Км" value={newRelocMileage} onChange={e => setNewRelocMileage(e.target.value ? Number(e.target.value) : "")} className="flex-1 bg-slate-700 text-white rounded px-2 py-2 sm:py-1 border border-slate-600 text-sm" />
              <input type="number" step="0.1" placeholder="₽/км" value={newRelocRate} onChange={e => setNewRelocRate(e.target.value ? Number(e.target.value) : "")} className="w-20 bg-slate-700 text-white rounded px-2 py-2 sm:py-1 border border-slate-600 text-sm text-center" />
              <button onClick={addRelocation} disabled={!newRelocMileage} className="bg-amber-600 disabled:bg-slate-600 text-white px-4 sm:px-3 py-2 sm:py-1 rounded min-h-[44px] sm:min-h-0"><Plus className="w-4 h-4" /></button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-r from-green-900/30 to-emerald-900/30 rounded-xl p-4 border border-green-500/30">
          <div className="space-y-1 text-sm mb-3">
            ...
            {totalFines > 0 && <div className="flex justify-between"><span className="text-slate-400">Штрафы</span><span className="text-red-400">−{totalFines.toLocaleString()} ₽</span></div>}
            {totalRelocations > 0 && <div className="flex justify-between"><span className="text-slate-400">Порожний пробег</span><span className="text-amber-400">−{totalRelocations.toLocaleString()} ₽</span></div>}
          </div>
